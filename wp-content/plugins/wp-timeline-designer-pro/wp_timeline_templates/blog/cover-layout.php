...
if ( 1 == $layout_type ) {
	...
	$animation = esc_attr( $wtl_animation );
}
?>
<div 
<?php
...
?>
class="wtl_blog_single_post_wrapp <?php echo esc_attr( Wtl_Template_Config::get_class_category( $wtl_settings ) ); ?>" id="wtl_date_<?php echo esc_attr( get_the_time( 'd' ) ); ?>_<?php echo esc_attr( get_the_time( 'm' ) ); ?>">
	<?php
	if ( 2 == $layout_type ) {
		$defalt_icon = Wtl_Template_Cover_Layout::get_default_icon( $wtl_settings );
		if ( $defalt_icon && 1 != $wtl_settings['hide_timeline_icon'] ) {
			$dicon = $defalt_icon;
		} else {
			$dicon = '';
		}
	...
	if ( 2 == $layout_type ) {
		if ( $wtl_icon ) {
			...
			if ( $dicon ) {
				...
				echo '<div class="wtl_steps_post_format"><i class="' . esc_attr( $dicon ) . '"></i></div>';
			}
		}
	}
	...
	<div class="wtl_cover_layout_inner">
		...
		<div class="wtl-post-footer">
			<?php
			Wtl_Template_Config::get_tags( $wtl_settings, false, false );
