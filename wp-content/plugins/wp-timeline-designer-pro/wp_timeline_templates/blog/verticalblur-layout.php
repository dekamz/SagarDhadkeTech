 ...
 * This template can be overridden by copying it to yourtheme/wp_timeline_templates/blog/verticalblur-layout.php.
 *
...
?>
	<style>
		<?php
		$title_color = Wtl_Template_Verticalblur_Layout::post_background_color();
		if ( $title_color ) {
	...
	?>
	<div id="wtl_date_<?php echo esc_attr( get_the_time( 'd' ) ); ?>_<?php echo esc_attr( get_the_time( 'm' ) ); ?>" class="wtl-schedule-post-content <?php echo 'post_' . esc_attr( Wtl_Template_Verticalblur_Layout::even_odd( $i ) ); ?>">
		<div data-aos="<?php echo esc_attr( $wtl_animation ); ?>" id="wtl_layout_<?php echo esc_attr( $post->ID ); ?>" class="wtl_layout_<?php echo esc_attr( Wtl_Template_Verticalblur_Layout::even_odd( $i ) ); ?>_<?php echo esc_attr( $post->ID ); ?> wtl-schedule-all-post-content wtl_post_content_schedule aos-init aos-animate">
		<?php
		...
		?>
			...
			if ( 5 != $timeline_style_type ) {
				Wtl_Template_Verticalblur_Layout::get_post_image( $wtl_settings );
			}
			if ( 'minima' !== $timeline_style_view ) {
				?>
				<?php if ( isset( $wtl_settings['display_date'] ) && 1 == $wtl_settings['display_date'] ) { ?>
		...
		<?php } ?>
			...
			<div class="wtl-post-content" <?php echo esc_attr( $post_content_color_set ); ?>>
				...
				if ( 5 == $timeline_style_type ) {
					echo wp_kses( Wtl_Template_Verticalblur_Layout::get_post_date( $wtl_settings, false ), Wp_Timeline_Public::args_kses() );
					Wtl_Template_Verticalblur_Layout::get_post_image( $wtl_settings );
					Wtl_Template_Config::get_title( $wtl_settings );
				}
				...
				<div class="wtl-post-footer">
					...
					echo wp_kses( Wtl_Template_Verticalblur_Layout::social_icon( $wtl_settings ), Wp_Timeline_Public::args_kses() );
					?>
				</div>
		</div>
	</div>
<script>
	(function($) {
		if( $('#wp-timeline-preview-box').length == 0 ) {
			$.fn.timeline = function() {
			var selectors = {
				id: $(this),
				item: $(this).find(".wtl-schedule-post-content"),
