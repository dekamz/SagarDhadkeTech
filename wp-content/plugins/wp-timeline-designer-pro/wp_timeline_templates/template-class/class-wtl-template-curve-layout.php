 ...
 */
class Wtl_Template_Curve_Layout {
	/**
				...
				if ( isset( $thumbnail ) && ! empty( $thumbnail ) ) {
					...
					if ( Wp_Timeline_Main::wtl_get_first_embed_media( $post->ID, $wtl_settings ) && 1 == $wtl_settings['rss_use_excerpt'] ) {
						...
						if ( 1 == $wp_timeline_lightbox ) {
							...
							<a rel="lightcase" data-rel="lightcase:my-slideshow" title="' . esc_attr( $post->post_title ) . '" class="open_up" href="' . esc_url( $url ) . '"><i class="fa fa-plus-circle" aria-hidden="true"></i></a>
						</div>';
						}
						echo '</figure>';
					}
...
$wtl_template_curve_layout = new Wtl_Template_Curve_Layout();
