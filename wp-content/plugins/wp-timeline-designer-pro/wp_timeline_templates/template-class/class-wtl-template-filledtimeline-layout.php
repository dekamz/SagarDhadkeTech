 ...
 */
class Wtl_Template_Filledtimeline_Layout {
	/**
	...
	public static function render( $layout_id, $wtl_settings, $template_wrapper ) {
		...
		if ( 1 == $layout_type ) {
			...
			$out .= '<div class="wtl_wrapper wp_timeline_post_list filledtimeline layout_id_' . $layout_id . ' ' . $slider_class . '">';
			$out .= '<div class="wtl_blog_template wtl_single_post_wrapp">';
			...
			$out .= '<div class="wtl_wrapper wp_timeline_post_list filledtimeline layout_id_' . $layout_id . ' ' . $slider_class . '">';
			$out .= '<div class="wtl_blog_template wtl_single_post_wrapp">';
			$out .= '<div class="stem-wrapper">	<div class="stem"></div> <div class="stem-background"></div>';
			$out .= '</div>';
			$out .= '<div class="wtl-schedule-wrap">';
			$out .= $template_wrapper;
			$out .= '</div>';
			$out .= '<div class="single-stem-icon scroll-to-top trigger-scroll-to-top"><i class="fa fa-eject" aria-hidden="true"></i></div>';
			$out .= '</div>';
			$out .= '</div>';
			return $out;
		}
		...
		if ( 1 == $layout_type ) {
			...
			jQuery(document).ready(function(){
				...
				$(".wtl_al_slider").slick({
					...
					<?php
					$display_timeline_bar = isset( $wtl_settings['display_timeline_bar'] ) ? $wtl_settings['display_timeline_bar'] : '';
	...
	public static function get_default_icon( $wtl_settings ) {
		...
		$defaut_icon = '<i class="fa fa-star"></i>';
	...
	public static function get_timeline_icon() {
		...
		if ( ! empty( $wtl_sdk ) ) {
			...
			if ( 'image' === $type ) {
				...
				if ( $img_src ) {
					return '<div class="wtl_steps_post_format wtl-post-center-image"><div class="stem-wrappericon"><img src="' . esc_url( $img_src ) . '"></div></div>';
				}
			} elseif ( 'fontawesome' === $type ) {
				$img_src = $wtl_sdk['wtl_single_timeline_icon'];
				if ( $img_src ) {
					return '<div class="wtl_steps_post_format wtl-post-center-image"><div class="stem-wrappericon"><i class="' . esc_attr( $img_src ) . '"></i></div></div>';
				}
			}
		}
	}
			...
			if ( 1 == $layout_type ) {
				...
				if ( $loop->have_posts() ) {
					...
					while ( have_posts() ) :
						the_post();
						?>
						<div class="wtl-post-center-image" data-date="<?php echo 'post-id_' . esc_attr( get_the_ID() ); ?>" data-slick-index="<?php echo esc_attr( $i ); ?>" id="wtl_layout_<?php echo get_the_ID(); ?>">
							<?php
							// $ar_year = get_the_time( 'Y' );
							...
							$wtl_icon = self::get_timeline_icon();
							if ( $wtl_icon ) {
								echo wp_kses( $wtl_icon, Wp_Timeline_Public::args_kses() );
							} else {
								if ( $dicon ) {
									echo '<div class="stem-wrappericon">' . wp_kses( $dicon, Wp_Timeline_Public::args_kses() ) . '</div>';
								} else {
									echo '<div class="stem-wrappericon"><i class="' . esc_attr( $post_format ) . '"></i></div>';
								}
							}
							?>
						</div>
...
$wtl_template_filledtimeline_layout = new Wtl_Template_Filledtimeline_Layout();
