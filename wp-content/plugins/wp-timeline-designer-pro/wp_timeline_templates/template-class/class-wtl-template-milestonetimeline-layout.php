 ...
 */
class Wtl_Template_Milestonetimeline_Layout {
	/**
	...
	public static function render( $layout_id, $wtl_settings, $template_wrapper ) {
		...
		$out .= '<div class="wtl_wrapper wp_timeline_post_list milestonetimeline_layout layout_id_' . $layout_id . '">';
	...
	public static function get_post_date( $wtl_settings, $show_icon ) {
		...
		if ( isset( $wtl_settings['display_date'] ) && 1 == $wtl_settings['display_date'] ) {
			...
			if ( 'F j, Y' == $date_format ) {
				$wp_timeline_date = $ar_year;
			}
			$out .= '<div class="wtl-post-date">';
			$out .= ( $date_link ) ? '<a class="mdate" href="' . esc_url( get_day_link( $ar_year, $ar_month, $ar_day ) ) . '">' : '<span class="mdate">';
			$out .= '<span class="year">' . esc_html( $wp_timeline_date ) . '</span>';
			$out .= ( $date_format ) ? '</a>' : '</span>';
			$out .= '</div>';
			return $out;
		}
	}
	...
	public static function get_default_icon( $wtl_settings ) {
		...
		$defaut_icon = '<i class="fas fa-truck"></i>';
					...
					<div class="wtl-post-tags">
						<span class="tags <?php echo ( $tags_link ) ? 'wp_timeline_no_links' : 'wp_timeline_has_link'; ?>">
							...
							foreach ( $post_tags as $tag ) {
								...
								echo esc_html( $disable_link_tag ) ? '<div>' . esc_html( $tag->name ) : '<a href="' . esc_url( get_term_link( $tag->term_id ) ) . '">' . esc_html( $tag->name );
								echo ( $disable_link_tag ) ? '</div>' : '</a>';
								echo '</span>';
								$sep++;
							}
							?>
...
$wtl_template_milestonetimeline_layout = new Wtl_Template_Milestonetimeline_Layout();
