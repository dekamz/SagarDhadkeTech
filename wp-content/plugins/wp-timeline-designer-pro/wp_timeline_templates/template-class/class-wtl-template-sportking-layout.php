 ...
 */
class Wtl_Template_Sportking_Layout {
	/**
	...
	public static function render( $layout_id, $wtl_settings, $template_wrapper ) {
		...
		if ( 1 == $layout_type ) {
			$noofshow = $wtl_settings['posts_per_page'];
		}
		$out  = '';
		$out .= self::js( $wtl_settings );
		$out .= self::show_post_number( $wtl_settings );
		$out .= '<div class="wtl_wrapper wp_timeline_post_list sportking_layout layout_id_' . $layout_id . '">';
		$out .= '<div class="wtl_blog_template blog_template schedule wtl_single_post_wrapp wtl_show_post-'.$noofshow.'">';
	 ...
	 * @return void
	 */
	public static function show_post_number( $wtl_settings ) {
	}
	...
	public static function get_post_date( $wtl_settings, $show_icon ) {
		...
		if ( isset( $wtl_settings['display_date'] ) && 1 == $wtl_settings['display_date'] ) {
			...
			$out              = '';
			$out             .= '<span class="wtl-post-date demo">';
			if($show_icon){
				$out             .= '<i class="far fa-calendar"></i>';
			}							
			$out             .= ( $date_link ) ? '<a class="mdatedemo" href="' . esc_url( get_day_link( $ar_year, $ar_month, $ar_day ) ) . '">' : '<div class="mdate">';
			$out             .= '<span class="year">' . esc_html( $year ) . '</span>';
	...
	public static function get_post_image( $wtl_settings ) {
		...
		if ( isset( $wtl_settings['wp_timeline_enable_media'] ) ? $wtl_settings['wp_timeline_enable_media'] : '' ) {
			...
			if ( ! empty( $thumbnail_type ) ) {
				...
				?>
				<div class="wtl_date_cat">
					<?php
						echo wp_kses( self::get_post_date( $wtl_settings, false ), Wp_Timeline_Public::args_kses() );
					?>
				</div>
				<?php
				echo '</div>';
...
$wtl_template_sportking_layout = new Wtl_Template_Sportking_Layout();