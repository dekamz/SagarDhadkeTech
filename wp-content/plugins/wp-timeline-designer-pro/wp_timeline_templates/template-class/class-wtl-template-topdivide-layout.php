 ...
 */
class Wtl_Template_Topdivide_Layout {
	/**
	...
	public static function render( $layout_id, $wtl_settings, $template_wrapper ) {
		...
		$out .= '<div id="wtl_steps" class="wtl_wrapper wp_timeline_post_list top_divide_layout schedule_cover layout_id_' . $layout_id . '">';
	...
	public static function js( $wtl_settings ) {
		ob_start();
			...
			$display_timeline_bar = ( isset( $wtl_settings['display_timeline_bar'] ) && '' !== $wtl_settings['display_timeline_bar'] ) ? $wtl_settings['display_timeline_bar'] : 0;
		...
		?>
			...
			jQuery(document).ready(function(){
				...
				$('.wtl_al_nav').slick({
					...
					dots:false,
					infinite:false,
					nextArrow: '<span class="wtl-ss-right"><i class="fas fa-arrow-right"></i></span>',
					prevArrow: '<span class="wtl-ss-left"><i class="fas fa-arrow-left"></i></i></span>',
					// focusOnSelect:true,
					adaptiveHeight:true,
				...
				if ( $post_tags ) {
					...
					<div class="wtl-post-tags">
						<span class="tags <?php echo ( $tags_link ) ? 'wp_timeline_no_links' : 'wp_timeline_has_link'; ?>">
							<span class="link-lable"> <i class="fas fa-tags"></i> <?php esc_html_e( 'Tags', 'wp-timeline-designer-pro' ); ?>:&nbsp; </span>
						<?php
			...
			if ( $loop->have_posts() ) {
				...
				while ( $loop->have_posts() ) :
					...
					?>
						...
						echo '<span class="wtl_steps_post_year"> ' . esc_html( get_the_date() ) . ' </span>';
						if ( $defalt_icon ) {
							$dicon = $defalt_icon;
						} else {
							$dicon = '';
...
$wtl_template_topdivide_layout = new Wtl_Template_Topdivide_Layout();
