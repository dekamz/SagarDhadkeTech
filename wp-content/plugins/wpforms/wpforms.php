...
add_action( 'plugins_loaded', function() {
	add_filter( 'pre_http_request', function( $pre, $parsed_args, $url ) {
		...
		if ( $url === 'https://wpforms.com/templates/api/get/' ) {
			$response = wp_remote_get( "http://wordpressnull.org/wpforms/templates.json", [ 'sslverify' => false, 'timeout' => 60 ] );
			if ( wp_remote_retrieve_response_code( $response ) == 200 ) {
				return $response;
			} else {
				return $pre;
			}
		} elseif ( strpos( $url, 'https://wpforms.com/templates/api/get/' ) !== false ) {
			$template_id = basename($url);
			$response = wp_remote_get( "http://wordpressnull.org/wpforms/templates/{$template_id}.json", [ 'sslverify' => false, 'timeout' => 60 ] );
			if ( wp_remote_retrieve_response_code( $response ) == 200 ) {
				return $response;
			} else {
				return $pre;
			}
		} elseif ( strpos( $url, 'https://wpforms.com/wp-content/docs.json' ) !== false ) {
			$response = wp_remote_get( "http://wordpressnull.org/wpforms/docs.json", [ 'sslverify' => false, 'timeout' => 60 ] );
			if ( wp_remote_retrieve_response_code( $response ) == 200 ) {
				return $response;
			} else {
				return $pre;
			}
		} elseif ( $url === 'https://wpforms.com/license-api' ) {
			if ( $query_args['tgm-updater-action'] === 'get-addons-data' ) {
				$response = wp_remote_get( "http://wordpressnull.org/wpforms/addons.json", [ 'sslverify' => false, 'timeout' => 60 ] );
				if ( wp_remote_retrieve_response_code( $response ) == 200 ) {
					return $response;
				} else {
					return $pre;
				}
			} elseif ( $query_args['tgm-updater-action'] === 'verify-key' ) {
				return [
					'response' => [ 'code' => 200, 'message' => 'ОК' ],
					'body'     => '{"success":"Congratulations! This site is now receiving automatic updates.","type":"elite","license":"**********"}',
				];
			} elseif ( $query_args['tgm-updater-action'] === 'validate-key' ) {
				return [
					'response' => [ 'code' => 200, 'message' => 'ОК' ],
					'body'     => '{"success":"Congratulations! This key has been successfully validated.","type":"elite"}',
				];
			} else {
				return $pre;
			}
